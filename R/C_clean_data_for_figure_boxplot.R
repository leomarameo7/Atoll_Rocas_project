# Script to load and clean raw data set for Monte Carlos trials 
#####Call packages#####
library(readr)
library(tidyverse)
library(plyr)
...
#####Read raw data####
c <- read_csv("data/raw/raw_trials.csv", col_types = cols())
#####Colapse columns trials in only one named "biomass" and preserving the others#####
c <- gather(c, "trials", "biomass", 3:252)
#  Basic checks --------------------------------------------------------------
nrow(c)             # How many rows
head(c)             # First rows
any(duplicated(c))  # There is any duplicated rows?
any(is.na(c))       # There are NAs in the data?
...
#### Sum all fish species for 2050 year
g1 <- g %>% group_by(scenario,trials,year,species) %>% 
   summarise_all(funs(sum))
g1 <- g1[,-4]   
g1 <- g1[,-3]
g1 <- g1 %>% group_by(scenario,trials) %>% summarise_all(funs(sum))
#### Sum all fish species for 2100
d1 <- d %>% group_by(scenario,trials,year,species) %>% 
   summarise_all(funs(sum))
d1 <- d1[,-4]   
d1 <- d1[,-3]
d1 <- d1 %>% group_by(scenario,trials) %>% summarise_all(funs(sum))


#####Save the file #####
write.csv(d,file  = "data/processed/processed_data_figure_box-plot_year_2100_fish.csv", row.names = F)
write.csv(g1,file  = "data/processed/processed_data_figure_box-plot_year_2050_total_fish_biomass.csv", row.names = F)
write.csv(d1,file  = "data/processed/processed_data_figure_box-plot_year_2100_total_fish_biomass.csv", row.names = F)
