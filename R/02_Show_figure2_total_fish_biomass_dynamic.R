...
library(pillar)
####Read data#####
b <- as.data.frame(read_csv("data/processed/montecarlo_trials_mean_percentils.csv"))
...
b <- subset(b,species %in% c("X2","X3","X4","X5","X6",
                         "X7","X8","X9","X10","X11","X12","X13","X14","X15","X16","X17"))
##### Sum fish biomass , per year and scenario #####
b2 <- b %>% 
   group_by(year.group, scenario) %>% 
   summarise(tbmean=sum(mean), tbmin=sum(percent_05),tbmax=sum(percent_95))
######Rename Scenarios#####
b2$scenario <- replace(b2$scenario, b2$scenario == "baseline", "Status quo")
b2$scenario <- replace(b2$scenario, b2$scenario == "RCP2", "RCP 2.6")
b2$scenario <- replace(b2$scenario, b2$scenario == "RCP4", "RCP 4.5")
b2$scenario <- replace(b2$scenario, b2$scenario == "RCP8", "RCP 8.5")
b2$scenario <- factor(b2$scenario,levels=c("Status quo", "RCP 2.6","RCP 4.5","RCP 8.5"))
#####Plot figure 2#####
p <- ggplot(data= b2) + aes(x = year.group) + 
   ...
   scale_y_continuous(position = "left", limits=c(0,34), breaks=seq(0,34,2))
p
